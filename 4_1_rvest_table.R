...
tables <- html_nodes(doc, xpath = "//table") 
...
# by css selector
table <- html_node(doc, "#Main table") # the only one data.frame

# by xpath
table <- html_node(doc, xpath = '//*[@id="Main"]//table')
str(html_table(table))

heads <- html_text(html_nodes(table, "thead td"))
bodys <- html_text(html_nodes(table, "tbody td"))
